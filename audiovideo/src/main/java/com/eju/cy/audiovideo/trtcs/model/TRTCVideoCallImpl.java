...
public class TRTCVideoCallImpl implements ITRTCVideoCall {
    ...
    private TIMMessageListener mTIMMessageListener = new TIMMessageListener() {
        ...
        public boolean onNewMessages(List<TIMMessage> list) {
            ...
            Log.d(TAG, "onNewMessages:TRTCVideoCallImpl " + list.size());
            // 收到新消息，取最新的一条进行判断
            TIMMessage msg = list.get(0);
            CallModel callModel = convert2VideoCallData(msg);
            if (null != callModel) {

                LogUtils.w("收到消息----" + callModel.toString());
    ...
    private TRTCCloudListener mTRTCCloudListener = new TRTCCloudListener() {
        ...
        public void onRemoteUserEnterRoom(String userId) {
            ...
            LogUtils.w("移除未响应定时器" + userId);
            LogUtils.w("现在多少人？？" + mCurRoomUserSet.size());
            // ToastUtils.showLong("移除的用户是" + userId);
        ...
        public void onRemoteUserLeaveRoom(String userId, int reason) {
            ...
            LogUtils.w("还有多少人？？" + mCurRoomUserSet.size());
            preExitRoom();
            if (mTRTCInteralListenerManager != null) {
     ...
     * 有我就处理
     *
     * @param callModel   自定义数据
     * @param user        发送方用户IM ID
     * @param timeoutTime
     */
    private void handleDialing(CallModel callModel, String user, long timeoutTime) {
        // 与对方处在同一个聊天室中，此时收到了别人的邀请列表
        //  LogUtils.w("user---" + user);
        //LogUtils.w("invitedList---" + callModel.invitedList);
        if (mCurCallID.equals(callModel.callId) && mCurGroupId.equals(callModel.groupId)) {
            //循环被邀请人ID
            mCurInvitedList = callModel.invitedList;
            for (final String id : mCurInvitedList) {
                if (!mTimeoutMap.containsKey(id)) {
                    LogUtils.w("同步列表:" + id);
                    // if (!AppConfig.appImId.equals(id)) {
            ...
            if (mTRTCInteralListenerManager != null) {
                ...
                mTRTCInteralListenerManager.onGroupCallInviteeListUpdate(mCurInvitedList, callModel.jurisdictionList);
            }
            return;
        }
        ...
        if (!TextUtils.isEmpty(mCurGroupId)) {
            ...
            for (String id : mCurInvitedList) {
                //  if (!id.equals(AppConfig.appImId)) {
                // if (!AppConfig.appImId.equals(id)) {
        ...
        //删除自己的超时回调
        //  removeInvitedTimeoutCallBack(AppConfig.appImId);
        if (mTRTCInteralListenerManager != null) {
            mTRTCInteralListenerManager.onInvited(user, onInvitedUserListParam, callModel.jurisdictionList, !TextUtils.isEmpty(mCurGroupId), mCurGroupId, mCurCallType);
            LogUtils.w("onInvited" + onInvitedUserListParam.size());
        }
        // 开始超时处理
    ...
    public void addInviteTimeoutCallback(final String userId, final String callId, final String groupId, final long timeOut, String ds) {
        LogUtils.w("添加超时处理的有---" + userId);
        final Runnable runnable = new Runnable() {
            @Override
            public void run() {
    ...
    private void removeInvitedTimeoutCallBack(String userId) {
        LogUtils.w("removeInvitedTimeoutCallBack" + userId);
     ...
     */
    @Override
    public void groupCall(final List<String> userIdList, List<String> jurisdictionList, int type, String groupId) {
        if (isCollectionEmpty(userIdList)) {
            return;
        }
        internalCall(userIdList, jurisdictionList, type, groupId);
    }
     ...
     */
    private void internalCall(final List<String> userIdList, List<String> jurisdictionList, int type, String groupId) {
        // 主动拨打电话，开始监听trtc的消息
        mTRTCCloud.setListener(mTRTCCloudListener);
        ...
        mLastCallModel.jurisdictionList = jurisdictionList;
        ...
        // 为邀请的id增加超时计时器
        for (final String userId : filterInvitedList) {
            // if (!AppConfig.appImId.equals(userId)) {
    ...
    private void preExitRoom() {
        LogUtils.w("preExitRoom:--- " + mCurRoomUserSet.toString() + " " + mCurInvitedList.toString());
    ...
    public void hangup() {
        ...
        if (fromGroup) {
            LogUtils.w("挂断1111" + mCurGroupId);
            groupHangup();
        } else {
            LogUtils.w("挂断2222" + mCurGroupId);
            singleHangup();
        }
    }
    ...
    private void groupHangup() {
        ...
        if (null != mCurRoomUserSet && mCurRoomUserSet.size() > 0) {
    ...
    private void singleHangup() {
        for (String id : mCurInvitedList) {
            sendModel(id, CallModel.VIDEO_CALL_ACTION_HANGUP);
        }
        stopCall();
        exitRoom();
    }
    ...
    public static class CallModel implements Cloneable, Serializable {
         ...
         * 有权限开视频的用户
         */
        @SerializedName("jurisdiction_list")
    ...
    private class TRTCInteralListenerManager implements TRTCVideoCallListener {
        ...
        public void onInvited(String sponsor, List<String> userIdList, List<String> jurisdictionList, boolean isFromGroup, String groupId, int callType) {
            for (WeakReference<TRTCVideoCallListener> reference : mWeakReferenceList) {
                TRTCVideoCallListener listener = reference.get();
                if (listener != null) {
                    listener.onInvited(sponsor, userIdList, jurisdictionList, isFromGroup, groupId, callType);
                    LogUtils.w("onInvited" + userIdList.size());
                }
            }
        }
        ...
        public void onGroupCallInviteeListUpdate(List<String> userIdList, List<String> jurisdictionList) {
            for (WeakReference<TRTCVideoCallListener> reference : mWeakReferenceList) {
                TRTCVideoCallListener listener = reference.get();
                if (listener != null) {
                    listener.onGroupCallInviteeListUpdate(userIdList, jurisdictionList);
                }
            }
        }
    ...
    @Override
    public void removeInvited(String mCurUserId) {
        removeInvitedTimeoutCallBack(mCurUserId);
    }
}
