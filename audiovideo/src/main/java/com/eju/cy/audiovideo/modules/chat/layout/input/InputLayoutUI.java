package com.eju.cy.audiovideo.modules.chat.layout.input;
...
import com.eju.cy.audiovideo.modules.chat.base.BaseInputFragment;
import com.eju.cy.audiovideo.modules.chat.interfaces.IInputLayout;
...
abstract class InputLayoutUI extends LinearLayout implements IInputLayout {
    ...
    protected List<InputMoreActionUnit> mInputMoreActionList = new ArrayList<>();//默认
    protected List<InputMoreActionUnit> mInputMoreCustomActionList = new ArrayList<>();//自定义
    protected List<InputMoreActionUnit> mInputMoreActionShowList = new ArrayList<>();//最终显示
...
//        mShortcutArea = findViewById(R.id.shortcut_area);
        ...
        ll_input_bg = findViewById(R.id.ll_input_bg);
        // 子类实现所有的事件处理
        init();
    }
    ...
    @Override
    public void setActionShow(List<String> list) {

//        mInputMoreActionShowList.clear();
//        InputMoreActionUnit action = new InputMoreActionUnit();
//        for (String str : list) {
//
//
...
//                        }
//                    });
//                    mInputMoreActionShowList.add(action);
//
//                    break;
//                case "拍照":
//                    action = new InputMoreActionUnit();
...
//                case "名片":
//
//                    for (InputMoreActionUnit inputMoreActionUnit : mInputMoreCustomActionList) {
//                        if (inputMoreActionUnit.getTitleId() == R.string.business_card) {
//                            mInputMoreActionShowList.add(inputMoreActionUnit);
//                        }
//
//                    }
//
...
//                case "房源":
//                    for (InputMoreActionUnit inputMoreActionUnit : mInputMoreCustomActionList) {
//                        if (inputMoreActionUnit.getTitleId() == R.string.housing) {
//                            mInputMoreActionShowList.add(inputMoreActionUnit);
//                        }
//
//                    }
...
//                case "音频通话":
//                    for (InputMoreActionUnit inputMoreActionUnit : mInputMoreCustomActionList) {
//                        if (inputMoreActionUnit.getTitleId() == R.string.audio_call) {
//                            mInputMoreActionShowList.add(inputMoreActionUnit);
//                        }
//
//                    }
...
//            }
     ...
     * @param msgType
     */
    public void setMsgType(MsgTypeEnmumer msgType) {
    ...
    public void setShowMenu() {
        ...
        List<String> actionMenuList = new ArrayList<>();
        if (this.msgType.equals(MsgTypeEnmumer.C2C.value())) {
            actionMenuList.clear();
            actionMenuList.addAll(EjuImController.getInstance().getC2CInputMoreCustomActionList());
        } else {
            actionMenuList.clear();
            actionMenuList.addAll(EjuImController.getInstance().getGroupInputMoreCustomActionList());
        }


        for (String str : actionMenuList) {
            ...
            switch (str) {
                ...
                case "相册":
                    InputMoreActionUnit actionPic = new InputMoreActionUnit();
                    if (AppConfig.appType == 0) {
                        actionPic.setIconResId(R.drawable.icon_fm_xc);
                    } else {
                        actionPic.setIconResId(R.drawable.ic_more_picture);
                    }

                    actionPic.setTitleId(R.string.pic);
                    actionPic.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            startSendPhoto();
                        }
                    });
                    mInputMoreActionShowList.add(actionPic);
                    break;
                case "拍照":
                    InputMoreActionUnit actionPhoto = new InputMoreActionUnit();

                    if (AppConfig.appType == 0) {
                        actionPhoto.setIconResId(R.drawable.icon_mf_sx);
                    } else {
                        actionPhoto.setIconResId(R.drawable.ic_more_camera);
                    }

                    actionPhoto.setTitleId(R.string.photo);
                    ...
                    mInputMoreActionShowList.add(actionPhoto);
                    break;
                case "摄像":
                    InputMoreActionUnit actionVideo = new InputMoreActionUnit();

                    if (AppConfig.appType == 0) {
                        actionVideo.setIconResId(R.drawable.icon_mf_sx);
                    } else {
                        actionVideo.setIconResId(R.drawable.ic_more_camera);
                    }

                    actionVideo.setTitleId(R.string.video);
                    actionVideo.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            startCapture();
                        }
                    ...
                    mInputMoreActionShowList.add(actionVideo);
                    break;
                case "文件":
                    InputMoreActionUnit actionFile = new InputMoreActionUnit();

                    if (AppConfig.appType == 0) {
                        actionFile.setIconResId(R.drawable.icon_mf_file);
                    } else {
                        actionFile.setIconResId(R.drawable.ic_more_file);
                    }

                    actionFile.setTitleId(R.string.file);
                    actionFile.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            startSendFile();
                    ...
                    mInputMoreActionShowList.add(actionFile);
                    break;
        ...
        LogUtils.w("actionMenuList----" + actionMenuList.size() + "\nmsgType-----" + this.msgType + "\nmInputMoreActionShowList---" + mInputMoreActionShowList.size() + "\nmInputMoreCustomActionList----" + mInputMoreCustomActionList.size());


    }

    /**
     * 设置setInputLayoutBg背景
     *
     * @param bg
     */
    public void setInputLayoutBg(Drawable bg) {
        ll_input_bg.setBackground(bg);
    }
}
