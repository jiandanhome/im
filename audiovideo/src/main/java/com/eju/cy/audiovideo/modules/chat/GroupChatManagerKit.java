...
//            //  ToastUtil.toastLongMessage("您已被同意加入圈子：" + groupSysEle.getGroupId());
//
//            ToastUtil.toastLongMessage("您已被同意加入圈子：" + groupSysEle.getGroupId());
//
//
//        } else if (type == TIMGroupSystemElemType.TIM_GROUP_SYSTEM_ADD_GROUP_REFUSE_TYPE) {
//            ToastUtil.toastLongMessage("您被拒绝加入圈子：" + groupSysEle.getGroupId());
//        } else if (type == TIMGroupSystemElemType.TIM_GROUP_SYSTEM_KICK_OFF_FROM_GROUP_TYPE) {
//            ToastUtil.toastLongMessage("您已被踢出圈子：" + groupSysEle.getGroupId());
...
//            ToastUtil.toastLongMessage("您所在的圈子" + groupSysEle.getGroupId() + "已解散");
//            if (mCurrentChatInfo != null && groupSysEle.getGroupId().equals(mCurrentChatInfo.getId())) {
//                onGroupForceExit();
//            }
//        }

        getGroupInfoByID(groupSysEle.getGroupId(), groupSysEle);
    }
     ...
     * @param groupSysEle
     */
    private void getGroupInfoByID(String groupId, final TIMGroupSystemElem groupSysEle) {


        GroupController.getInstance().getGroupInfo(groupId, new ImCallBack() {
            @Override
            ...
            public void onSuccess(Object var1) {
                ...
                if (null != timGroupDetailInfoResults && timGroupDetailInfoResults.size() > 0) {
                    ...
                    if (type == TIMGroupSystemElemType.TIM_GROUP_SYSTEM_ADD_GROUP_ACCEPT_TYPE) {
                        ...
                        ToastUtil.toastLongMessage("您已被同意加入圈子：" + timGroupDetailInfoResult.getGroupName());


                    } else if (type == TIMGroupSystemElemType.TIM_GROUP_SYSTEM_ADD_GROUP_REFUSE_TYPE) {
                        ToastUtil.toastLongMessage("您被拒绝加入圈子：" + timGroupDetailInfoResult.getGroupName());
                    } else if (type == TIMGroupSystemElemType.TIM_GROUP_SYSTEM_KICK_OFF_FROM_GROUP_TYPE) {
                        ToastUtil.toastLongMessage("您已被踢出圈子：" + timGroupDetailInfoResult.getGroupName());
                        ConversationManagerKit.getInstance().deleteConversation(groupSysEle.getGroupId(), true);
                        if (mCurrentChatInfo != null && groupSysEle.getGroupId().equals(mCurrentChatInfo.getId())) {
                            onGroupForceExit();
                        }
                    } else if (type == TIMGroupSystemElemType.TIM_GROUP_SYSTEM_DELETE_GROUP_TYPE) {
                        ToastUtil.toastLongMessage("您所在的圈子" + timGroupDetailInfoResult.getGroupName() + "已解散");
                        if (mCurrentChatInfo != null && groupSysEle.getGroupId().equals(mCurrentChatInfo.getId())) {
                            onGroupForceExit();
                        }
                    }
