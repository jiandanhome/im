package com.eju.cy.audiovideo.modules.contact;
...
public class ContactAdapter extends RecyclerView.Adapter<ContactAdapter.ViewHolder> {
  ...
  //  private int position;
    ...
    @Override
    public void onBindViewHolder(final ContactAdapter.ViewHolder holder,  int position1) {

        final ContactItemBean contactBean = mData.get(holder.getAdapterPosition());
        ...
        if (holder.getAdapterPosition() < mData.size() - 1) {
            ...
            String tag2 = mData.get(holder.getAdapterPosition() + 1).getSuspensionTag();
        ...
        if (mOnSelectChangedListener != null) {
            ...
            holder.ccSelect.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    mOnSelectChangedListener.onSelectChanged(getItem(holder.getAdapterPosition()), isChecked);
                }
            });
        }
        ...
        holder.content.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (mOnClickListener != null) {
                    mOnClickListener.onItemClick(holder.getAdapterPosition(), contactBean);
                }
                if (isSingleSelectMode && holder.getAdapterPosition() != mPreSelectedPosition && contactBean.isSelected()) {
                    //单选模式的prePos处理
                    mData.get(mPreSelectedPosition).setSelected(false);
                    notifyItemChanged(mPreSelectedPosition);
                }
                mPreSelectedPosition = holder.getAdapterPosition();
            }
        });
