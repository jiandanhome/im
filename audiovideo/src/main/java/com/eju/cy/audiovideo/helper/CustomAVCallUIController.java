    ...
    public void onDraw(ICustomMessageViewGroup parent, CustomMessage data) {
        ...
        switch (data.getAction()) {
            ...
            case VIDEO_CALL_ACTION_REJECT:
                if (null!=data.getUserId()&&data.getUserId().equals(AppConfig.appImId)) {
                    callingAction = "[已拒绝]";
...
//    }
    ...
    public void sendVideoCallAction(int action, CustomMessage roomInfo) {
        ...
        //非拨打 调换下逻辑
         if (action == VIDEO_CALL_ACTION_DIALING||action == VIDEO_CALL_ACTION_SPONSOR_TIMEOUT||action == VIDEO_CALL_ACTION_SPONSOR_CANCEL||action == VIDEO_CALL_ACTION_HANGUP||action == VIDEO_CALL_ACTION_REJECT) {
                            ...
                            if (null != userId && !"".equals(userId))
                                UserInfoController.getInstance().getUsersProfile(userId, true, new ImCallBack() {
                                    ...
                                    public void onSuccess(Object var1) {
                                        ...
                                        assembleOnlineCall(null, AppConfig.appImId, timUserProfiles.get(0).getFaceUrl(), timUserProfiles.get(0).getNickName(), calledUserID, calledFaceUrl, calledName, roomDto.getData().getId(), true);
